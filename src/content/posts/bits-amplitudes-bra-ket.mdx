...
Specifically: if a qubit has amplitude $\alpha$ for $|0\rangle$ and amplitude $\beta$ for $|1\rangle$, then:

- Probability of measuring 0: $|\alpha|^2$
- Probability of measuring 1: $|\beta|^2$

The constraint is that $|\alpha|^2 + |\beta|^2 = 1$ — the probabilities must sum to 1.
...
These are called **kets**. The vertical bar and angle bracket are part of the notation, not decoration. These are the **basis states**: the simplest, most definite states a qubit can be in. Measuring $|0\rangle$ always gives 0; measuring $|1\rangle$ always gives 1.

A general single-qubit state is a **linear combination** of the basis states:

$$|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$$

where $\alpha$ and $\beta$ are complex numbers satisfying $|\alpha|^2 + |\beta|^2 = 1$. This is superposition — not "being both at once," but occupying a precise mathematical combination of the two basis states.

As a column vector, this is equivalent to writing:

$$|\psi\rangle = [\alpha,\,\beta]^\top$$

where $|0\rangle = [1,\,0]^\top$ and $|1\rangle = [0,\,1]^\top$. The ket is just a vector; the notation is a more expressive way to write it.

The **bra** is the conjugate transpose of the ket: $\langle\psi| = [\alpha^*,\,\beta^*]$. This lets us write inner products cleanly: $\langle\varphi|\psi\rangle$ is the inner product of state $\varphi$ with state $\psi$ — a bra on the left, a ket on the right, forming a "bracket." That is where the name comes from.

The normalization constraint, in bra-ket form, is just $\langle\psi|\psi\rangle = 1$.

## What a Qubit Is

We now have a precise answer to the question.

A qubit is a unit vector in a two-dimensional complex vector space. Its two components are amplitudes. Measuring it collapses it to one of the basis states, with probabilities given by the squared magnitudes of the corresponding amplitudes. The state before measurement is not vague or undefined — it is exactly $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$. The uncertainty is in the outcome of measurement, not in the state itself.
