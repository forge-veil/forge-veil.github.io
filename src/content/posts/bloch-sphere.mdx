...
Two free parameters — exactly the number needed to index the surface of a sphere. The parametrization we are about to write bakes normalization in automatically, so the two degrees of freedom correspond directly to angular coordinates: a polar angle θ and an azimuthal angle φ.
