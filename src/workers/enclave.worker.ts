...
async function handleQuery(queryText: string) {
  ...
  if (currentEngine === 'transformers' && tfGenerator) {
    const result = await tfGenerator(
      [
        { role: 'system', content: 'You are a concise assistant. Answer only what the user asks. Do not add hashtags, usernames, or extra commentary.' },
        { role: 'user', content: queryText },
      ],
      { max_new_tokens: 200, repetition_penalty: 1.3 }
    );
    // v3 chat format: [{generated_text: [{role, content}, ...]}]
    // v4 may return [{generated_text: "string"}] for non-chat or same chat format
    const output = result as Array<{ generated_text: Array<{ role: string; content: string }> | string }>;
    const gt = output[0]?.generated_text;
    if (Array.isArray(gt)) {
      responseText = (gt.at(-1) as { role: string; content: string })?.content ?? '(no response)';
    } else {
      responseText = String(gt ?? '(no response)');
    }
